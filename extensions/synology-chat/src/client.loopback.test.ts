...
describe("Synology Chat client loopback", () => {
  ...
  it("delivers authenticated text and frozen media through both July adapters", async () => {
    ...
    for (const result of [outboundText, outboundMedia, durableText, durableMedia]) {
      expect(result).toMatchObject({
        ...
        messageId: expect.stringMatching(/^sc-\d+$/),
        chatId: "42",
        receipt: { threadId: "42" },
      });
      // July exposes a local receipt identifier. Preserve that older adapter
      // contract; this backport changes the file_url authority, not receipts.
