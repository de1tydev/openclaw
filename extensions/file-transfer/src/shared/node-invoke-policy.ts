...
> {
  ...
  const result = await runCommandBuffered([tarBin, "-tzf", "-"], {
    input: tarBuffer,
  ...
  if (result.code !== 0) {
    return {
      ok: false,
      code: "ARCHIVE_ENTRIES_UNREADABLE",
      reason: `tar -tzf exited ${result.code}: ${projectBoundedTextTail(result.stderr.toString("utf8"), DIR_FETCH_ARCHIVE_LIST_ERROR_STDERR_CHARS)}`,
    };
  }
