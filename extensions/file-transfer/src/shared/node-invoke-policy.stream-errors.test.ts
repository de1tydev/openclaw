...
const { runCommandBufferedMock } = vi.hoisted(() => ({
  runCommandBufferedMock: vi.fn(),
}));

vi.mock("./buffered-command.js", () => ({
...
function mockCommandResult(overrides: Record<string, unknown> = {}) {
  runCommandBufferedMock.mockImplementationOnce(
    async (
      _argv: string[],
...
describe("dir.fetch archive policy process wrapper", () => {
  it("fails archive listing closed on wrapper errors", async () => {
    runCommandBufferedMock.mockRejectedValueOnce(new Error("policy listing read failed"));
  ...
  it("normalizes successful archive entries", async () => {
    ...
    expect(runCommandBufferedMock).toHaveBeenCalledWith(
      expect.any(Array),
      expect.objectContaining({ discardOutput: { stdout: true } }),
    );
  });
