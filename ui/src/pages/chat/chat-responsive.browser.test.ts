...
async function waitForContextRingEntrance(page: Page): Promise<void> {
  // CI can delay the first animation frame beyond a wall-clock sleep. Measure
  // settled geometry only after the browser's own animation timeline completes.
  await page.locator(".context-ring").evaluate(async (node) => {
...
describeBrowserLayout.concurrent("chat responsive browser layout", () => {
  ...
  it("keeps the expanded mobile composer tight, scrollable, and separated from the thread", async () => {
    ...
    try {
      ...
      await waitForContextRingEntrance(page);
