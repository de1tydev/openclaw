...
External terminals, node services, and non-Windows callers keep their existing routing. Explicit `--force`, `--wait`, or `--skip-deferral` also retain their existing behavior and validation; they do not implicitly enable `--safe`.
