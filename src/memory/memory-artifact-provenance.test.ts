...
describe("memory artifact provenance", () => {
  ...
  it("rejects quarantined content and aliases while retaining owner and hash-bound agent memory", async () => {
    await withStateDirEnv("openclaw-memory-artifact-", async ({ tempRoot }) => {
      const workspaceDir = path.join(tempRoot, "workspace");
      await mkdir(path.join(workspaceDir, "memory"), { recursive: true });
      const address = { workspaceDir, relativePath: "memory/2026-07-31.md" };
      const filePath = path.join(workspaceDir, address.relativePath);
      await writeFile(filePath, "owner note");
      await expect(isMemoryArtifactEligibleForAutomaticContext(address)).resolves.toBe(true);
      await recordMemoryArtifactWriteProvenance({
        ...address,
        contentBefore: "owner note",
        contentAfter: "agent note",
        originClass: "agent",
        observedAt: 1,
      });
      await writeFile(filePath, "agent note");
      await expect(isMemoryArtifactEligibleForAutomaticContext(address)).resolves.toBe(true);
      await writeFile(filePath, "changed outside reserved write");
      await expect(isMemoryArtifactEligibleForAutomaticContext(address)).resolves.toBe(false);
      await recordMemoryArtifactWriteProvenance({
        ...address,
        contentBefore: "changed outside reserved write",
        contentAfter: "quarantined",
        originClass: "untrusted",
        observedAt: 2,
      });
      await writeFile(filePath, "quarantined");
      await symlink(filePath, path.join(workspaceDir, "MEMORY.md"));
      await expect(
        isMemoryArtifactEligibleForAutomaticContext({ workspaceDir, relativePath: "MEMORY.md" }),
      ).resolves.toBe(false);
    });
  });
