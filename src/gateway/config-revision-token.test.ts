...
describe("Gateway config revision tokens", () => {
  it("persists one opaque domain-separated key without changing schema version", () => {
    ...
    expect(
      reopened.prepare("SELECT name FROM sqlite_master WHERE name = 'config_revision_keys'").get(),
    ).toBeDefined();
    ...
    expect(loadGatewayConfigRevisionProjector(stateOptions()).projectRawHash(rawHash)).not.toBe(
      rawToken,
    );
