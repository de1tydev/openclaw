...
export async function waitForGatewayHealthyRestart(params: {
  ...
  for (let attempt = 0; ; attempt += 1) {
    ...
    if (elapsedMs >= standardDeadlineMs || migrationDeadlineMs !== undefined) {
      // Update callers already own a finite command budget. Keep their first service refresh
      // alive for that full budget so a long startup migration is not killed by a redundant
      // restart; standalone lifecycle callers retain the migration/post-migration windows.
      const deadlineMs =
